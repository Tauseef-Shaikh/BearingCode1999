...
def create_entry(request):
    ...
    if request.method == 'POST':
        ...
        usrDisplayList =[grpEntry,optType,str(oddevenSubType)+str(genericSubType), codeNum, 0, DtTm.strftime('%Y-%m-%d %H:%M:%S')]
        ...
        if actionType  == 'add':
            ...
            if grpEntry == 'yes':
                ...
                if optType.strip() == 'category':
                    updateCategory(codeNum, codeVal)
                elif optType.strip() == 'singlebearing':
                    updateSingleBearing(codeNum, codeVal)
                elif optType.strip() == 'doublebearing':
                    updateDoubleBearing(codeNum, codeVal)
                elif optType.strip() == 'triplebearing':
                    updateTripleBearing(codeNum, codeVal)
                elif optType.strip() == 'oddeven' and oddevenSubType == 'exclusion':
                    updateOddEvenExclusion(codeNum, codeVal)
                elif optType.strip() == 'oddeven' and oddevenSubType == 'inclusion':
                    updateOddEvenInclusion(codeNum, codeVal)
                elif optType.strip() == 'generic' and genericSubType == 'single':
                    updateGenericSingle(codeNum, codeVal)
                elif optType.strip() == 'generic' and genericSubType == 'double':
                    updateGenericDouble(codeNum, codeVal)
                elif optType.strip() == 'doubledigit':
                    updateDoubleDigit(codeNum, codeVal)
                elif optType.strip() == 'settype':
                    updateSetType(codeNum, codeVal)
                elif optType.strip() == 'royalclub':
                    updateRoyalClub(codeNum, codeVal)
                else:
            ...
            if grpEntry == 'yes':
                ...
                with open (__package__+'/data/grpEntry.csv','w') as cf:                    
                    cf.write('GrpEntry,BearingType,SubType,Code,Quantity,DateTime,EntryNo')
                    cf.write('\n')
                    
                    for lCnt, line in enumerate(csvrdr):
                        print(line)                    
                        if lCnt == int(entryNo)-1:                                                
                            toWrt = grpEntry + ',' + optType + ',' + str(oddevenSubType)+str(genericSubType) +',' + str(codeNum) +',' + str(codeVal) +',' + DtTm.strftime('%Y-%m-%d %H:%M:%S')+',' + entryNo
                            cf.write(toWrt)
                            cf.write('\n')                         
                            continue
                        cf.write(line)                        
    ...
    print (usrDisplayDict)
    print (toEdit)
    return render(request, 'myapp/create_entry.html', {'data_json': data_json, 'data_json1': data_json1, 'usrDisplayDict': usrDisplayDict, 'codeVal':codeVal, 'codeB4Split': codeB4Split})
            ...
            for tmpBrCd in categoryDict[_]:
                ...
                if lstC <= 12:                            
                    ...
                    if tmpBrCd in usrDict:
                        ...
                        usrDict[tmpBrCd] = int(usrDict[tmpBrCd]) + int((codeVal*9) / 150)
                        lstC += 1
                    else:
                        usrDict[tmpBrCd] = int((codeVal*9) / 150)
                        lstC += 1                                
                    ...
                    if tmpBrCd in usrDict:                            
                        usrDict[tmpBrCd] = int(usrDict[tmpBrCd]) + int((codeVal*9) / 300)
                        lstC += 1
                    else:
                        usrDict[tmpBrCd] = int((codeVal*9) / 300)
                        lstC += 1                            
                    ...
                    if tmpBrCd in usrDict:                            
                        usrDict[tmpBrCd] = int(usrDict[tmpBrCd]) + int((codeVal*9) / 900)
                        lstC += 1
                    else:
                        usrDict[tmpBrCd] = int((codeVal*9) / 900)
                        lstC += 1
    ...
    if optType.strip() == 'category':
        ...
        if grpEntry == 'no':
            for eCd in codeNum.split('-'):
                ...
                #print(usrAdminDict[eCd])
    
    return (usrAdminDict)

def updateCategory(codeNum, codeVal):
    ...
    for _ in codeNum.split('-'):#7-8-9
        ...
        for tmpBrCd in categoryDict[_]:
            ...
            if lstC <= 12:
                ...
                if tmpBrCd in usrAdminDict:
                    usrAdminDict[tmpBrCd] = int(usrAdminDict[tmpBrCd]) - (int(usrAdminDict[tmpBrCd]) - int((codeVal*9) / 150))
                    lstC += 1
                else:
                    usrAdminDict[tmpBrCd] = int((codeVal*9) / 150)
                    lstC += 1
                ...
                if tmpBrCd in usrAdminDict: 
                    usrAdminDict[tmpBrCd] = int(usrAdminDict[tmpBrCd]) - (int(usrAdminDict[tmpBrCd]) - int((codeVal*9) / 300))
                    lstC += 1
                else:
                    usrAdminDict[tmpBrCd] = int((codeVal*9) / 300)
                    lstC += 1                            
                ...
                if tmpBrCd in usrAdminDict:
                    usrAdminDict[tmpBrCd] = int(usrAdminDict[tmpBrCd]) - (int(usrAdminDict[tmpBrCd]) - int((codeVal*9) / 900))
                    lstC += 1
                else:
                    usrAdminDict[tmpBrCd] = int((codeVal*9) / 900)
                    lstC += 1

def updateSingleBearing(codeNum, codeVal):
    ...
    for _ in codeNum.split('-'): #7-8-9        
        for tmpCode in _:
            for tmpBrCd in categoryDict[tmpCode]:
                ...
                if lstC <= 12:
                    if tmpBrCd in usrAdminDict:
                        usrAdminDict[tmpBrCd] = usrAdminDict[tmpBrCd] - (usrAdminDict[tmpBrCd] - codeVal)
                        lstC += 1
                    else:
                        usrAdminDict[tmpBrCd] = codeVal
                        lstC += 1

def updateDoubleBearing(codeNum, codeVal):
    for _ in codeNum.split('-'):        
        for tmpCode in _:            
            for tmpBrCd in categoryDict[tmpCode][12:21]:
                tmpBrCd = str(tmpBrCd)
...
def updateTripleBearing(codeNum, codeVal):                                
    for _ in codeNum.split('-'):        
        for tmpCode in _:            
            for tmpBrCd in categoryDict[tmpCode][-1:]:
                tmpBrCd = str(tmpBrCd)
...
def updateOddEvenExclusion(codeNum, codeVal):
    for _ in codeNum.split('-'):                    
        for tmpCode in _:            
            for tmpBrCd in oddevenExc[tmpCode]:
                tmpBrCd = str(tmpBrCd)
...
def updateOddEvenInclusion(codeNum, codeVal):                                        
    for _ in codeNum.split('-'):        
        for tmpCode in _:            
            for tmpBrCd in oddevenInc[tmpCode]:
                tmpBrCd = str(tmpBrCd)
...
def updateGenericSingle(codeNum, codeVal):
    for _ in codeNum.split('-'):        
        for tmpCode in _:            
            for tmpBrCd in genSingle[tmpCode]:
                tmpBrCd = str(tmpBrCd)
...
def updateGenericDouble(codeNum, codeVal):
    for _ in codeNum.split('-'):        
        for tmpCode in _:            
            for tmpBrCd in genDouble[tmpCode]:
                tmpBrCd = str(tmpBrCd)
...
def updateSetType(codeNum, codeVal):
    print('updateSetType inside')
    for tmpCode in codeNum.split('-'):
        for tmpBrCd in setDict[tmpCode]:
            tmpBrCd = str(tmpBrCd)
            if usrAdminDict[tmpBrCd] != 0:
                usrAdminDict[tmpBrCd] = usrAdminDict[tmpBrCd] - codeVal
            elif usrAdminDict[tmpBrCd] == 0:
                usrAdminDict[tmpBrCd] = codeVal

def updateDoubleDigit(codeNum, codeVal):                
    for tmpCode in codeNum.split('-'):
        for tmpBrCd in doubleDigit[tmpCode]:
            tmpBrCd = str(tmpBrCd)
            if tmpBrCd in usrAdminDict:
...
def updateRoyalClub(codeNum, codeVal):
    for rcBrCd in codeNum.split('-'):
        rcCat = royalClub[rcBrCd]  #string 128 got int 1
        rcCat = str(rcCat)
        for revCd in royalClubRev[rcCat]:            
            revCd = str(revCd)            
            if revCd in usrAdminDict:
                usrAdminDict[revCd] = usrAdminDict[revCd] - (usrAdminDict[revCd] - codeVal)
            else:
                usrAdminDict[revCd] = codeVal
...
def admin_view(request):    
    ...
    context = {'table_data': table_data, 'usrAdminDict': json.dumps(usrAdminDict), 'categoryDict': categoryDict, 'catDictTotal':catDictTotal, 'sideTotal': sideTotal, 'nonZeroCdCnt': nonZeroCdCnt}
...
def create_breaks(request):
    ...
    #print('userInp -->', nonZeroKeyList)
    lst2Write, tmplst2Write = [],[]
    
    if userInp is not None:
        
        for rndCnt in userInp.split(','):            
...
genSingle={'0':[120,130,140,150,160,170,180,190,230,240,250,260,270,280,290,340,350,360,370,380,390,450,460,470,480,490,560,570,580,590,670,680,690,780,790,890],
    ...
    '5':[125,135,145,150,156,157,158,159,235,245,250,256,257,258,259,345,350,356,358,359,375,450,456,457,458,459,560,567,568,569,570,578,579,580,589,590],
...
with open (__package__+'/data/grpEntry.csv','w') as cf:
    cf.write('BearingType,SubType,Code,Quantity,DateTime,EntryNo')
    cf.write('\n')